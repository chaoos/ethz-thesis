...
However, the high setup cost presents challenges for gauge field sampling, where the gauge field changes frequently.
...
%Multigrid preconditioning comes with a high setup cost of generating the multigrid subspace with approximate low modes.
...
Now, we are ready to discuss the propagator decomposition using $\Nlvl$ levels of multigrid.
...
%\worktodo{
%* Summary
%  * mg precond treats the low modes suppressed by kylov solvers
%  * V2 prob
%  * mg vs. luschers inexact deflation
%  * coarse op move data faster through lattice
%  * mg prop decomp
%  * mg n-pt func decomp
%  * 2pt 2-lvl example
%  * estimator on levels
%  * LMA
% * critical discussion, limits
%   * recursice vs loop: different features regarding smoothing and flexibility
%   * coarse ops: same sparsity pattern, but less sparse, potantial for inefficiency
%   * bad strong scaling due to reduced problem size, hard on GPUs!
%   * many algorithmic parameters like number of level, block sizes, low modes, spins, etc ...
%   * HMC problematic
% }
...
Multigrid scales close to linear in the lattice volume, even though the dimension of the low-mode subspace does so too, because it harnesses the local coherence of low modes as of \cref{ch:p2:lc}.
...
\tldr{recursice vs loop: different features regarding smoothing and flexibility}
...
Or alternatively through a formulation where every level is a coarsening of the fine grid only, giving more flexibility.

\tldr{vrm prefers loop}
...
The former on the other hand is preferred for preconditioning where inter-grid data movement is crucial.
...
%Although, the variance reduction method builds elegantly on top of existing multigrid implementations by harnessing the existing building blocks, its usefulness might be limited due to the preference of the method for higher flexibility.

\tldr{coarse ops: same sparsity pattern, but less sparse, potential inefficient}
...
Solving these inefficiencies requires further algorithmic treatment, resulting in higher overall algorithm complexity.

\tldr{bad strong scaling due to reduced problem size, hard on GPUs}
Furthermore, coarse systems have a significantly reduced problem size.
...
\tldr{many algorithmic parameters like number of level, block sizes, low modes, spins, etc}
The introduction of multigrid comes with many new algorithmic parameters to take into consideration.
Most of them are not trivial to tune, such as the number of levels, block sizes, spins and colors on each level or smoothing choices between levels.
Every algorithm based on multigrid requires careful tuning and understanding of the effect of these parameters on performance, efficiency and stability.
With the newly introduced variance reduction method, their effect on variance contribution must be considered too.

The next chapter will numerically investigate the novel variance reduction method applied to the connected LO-HVP, as introduced in \cref{ch:p2:2pt-corr}, compare it to standard stochastic and LMA estimators, and determine the cost to reach minimal variance.
