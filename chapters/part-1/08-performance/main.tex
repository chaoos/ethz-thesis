...
The goal is to determine production readiness of the code to run on current accelerator-based super-computing centers, focusing on good performance on the very recent GH200 node at CSCS~\cite{fusco2024}.
...
Its contribution was insignificant in all cases and can even be pushed down further by increasing the number of nodes due to its perfect strong scaling.
...
However, performing runs with more than \num{32} GH200 nodes is not advisable because we are clearly in the bad strong scaling region then unless we study larger lattices as the ones considered or increase the number of RHSs substantially.
