...
This section describes the continuous integration testing and deployment (CI/CD) pipeline as introduced in \openqxd's development repository\cite{gitlab:openqxd-devel}.
...
This chapter details our strategy for continuous integration in \cref{sec:cicd:integration}, code testing philosophy in \cref{sec:cicd:testing} and direct software deployment to CSCS infrastructure in \cref{sec:cicd:deployment}.
...
We download the latest developments committed to the \code{develop} branch, i.e. their CI branch.
...
\begin{table}[t]
  ...
  \caption{\label{tab:cicd:ensembles}%
    Ensembles used in the CI/CD pipeline. The periodic lattice D5d is taken from \crefcite{online:cls}, whereas the C$^{\star}$ lattice is taken from \crefcite{RCstar22}.}
\end{table}
...
\end{minted}
where \code{<N>} is the number of the check we run, i.e. the check corresponds to \code{devel/quda/check<N>.c} which was built in the previous stage.
...
\end{minted}
We see that it simply extends the runner \code{.uenv-builder-daint-gh200}, which is provided by CSCS and included a few lines above.
