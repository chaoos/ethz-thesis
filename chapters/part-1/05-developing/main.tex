...
\tldr{develop intro}
This section describes how to easily port an existing program within the framework of \openqxd to use the solvers available through the interface to \quda from the viewpoint of an \openqxd application level developer.
...
If we build without the flag \code{QUDA\_INTERFACE}, the highlighted parts are removed and the code falls back to the standard function that dispatches the available solvers present in \openqxd
\footnote{If then called with a \quda solver, it will gracefully exit with a message \capcode{Unknown solver QUDA}.}.
...
The variable \code{id} in the code snippet is the solver identifier from the input file.
...
\begin{itemize}[leftmargin=77pt]
  ...
  \item[\Cref{sec:develop:openqcd:way}:] Ideal to interface existing programs.
...
This means that the programmer can just use the functions \code{read\_solver\_parms} to read solver sections from the input file, \code{print\_solver\_parms} to print information about parsed the solvers, \code{write\_solver\_parms} to write solver settings to disk and \code{check\_solver\_parms} to compare solver setups.
All these functions work with \quda solvers as well.
For more details about these function see \cref{sec:interface:legacy}.
...
For the multiple right-hand sides solver kernel the same concepts as for the standard kernel hold as it can be called as described before in \cref{sec:develop:sync:way,sec:develop:openqcd:way}.
...
However this coding pattern corresponds to \cref{fig:develop:serial} where we see an alternating idling pattern for the CPU and the GPU.
This does not fully utilize the available resources.
\begin{codelisting}
\begin{minted}[]{c}
for (int i = 0; i < Nsrc; ++i) {
...
Between we see the filled pipeline which solves the current set of sources and contracts the last set of solutions concurrently.
Notice that the contraction between the start and wait function must use the returned MPI communicator \code{comm}\footnote{We note that the global communicator \fncode{MPI\_COMM\_WORLD} provided by MPI to contain all processes will not work as expected in the above scenario between the start and wait functions and should be avoided, because by definition all threads belong to this communicator. Communication using this communicator will most certainly error or misbehave on strict MPI implementations.}.
Whatever communicator contained all ranks before the asynchronous calls (including \code{MPI\_COMM\_WORLD}) may safely be used again after the threads have joined, i.e. after the wait function returned.
\begin{codelisting}
\begin{minted}[]{c}
...
A field transfer consists of data reordering and memory copy between host and device.
