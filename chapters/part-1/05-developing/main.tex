...
\footnote{If then called with a \quda solver, it will gracefully exit with a message \code{Unknown solver QUDA}.}.
...
Notice that the contraction between the start and wait function must use the returned MPI communicator \code{comm}\footnote{We note that the global communicator \code{MPI\_COMM\_WORLD} provided by MPI to contain all processes will not work as expected in the above scenario between the start and wait functions and should be avoided, because by definition all threads belong to this communicator. Communication using this communicator will most certainly error or misbehave on strict MPI implementations.}.
