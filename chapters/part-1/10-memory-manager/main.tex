...
\end{codelisting}
This will register a signal handler \code{handler} that receives additional information triggered by the \code{SA\_SIGINFO} flag from the \code{siginfo\_t} struct, namely the offending pointer address in \code{info->si\_addr} among many others.
...
%Furthermore, although the approach works conceptually and has been successfully verified on a small scale, an alternative scheme employing explicit transfers could be considered too.
%In addition, explicit transfers allow one to distinguish between read and write accesses, thereby eliminating unnecessary data movement.
%They further allow to distinguish read and write accesses on the CPU, eliminating unnecessary transfers.
