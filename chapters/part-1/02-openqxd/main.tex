...
#define NPROC3_BLK 1
\end{minted}
\caption{Excerpt of the file \capcode{include/global.h} describing the \openqxd process grid.}
...
Furthermore, there is a single precision variant of this struct called \code{spinor} holding structs of type \code{su3\_vector} again holding \code{complex}.
...
\end{minted}
\caption{The \capcode{complex} and \capcode{complex\_dble} struct}
...
The gauge field has indices $(x_{odd}, \pm \mu, a, b)$, where $x_{odd}$ is the lattice index (only odd points, $V_\mathrm{L}/2$ elements), $\mu \in \{\pm 0, \pm 1, \pm 2, \pm 3\}$ is the direction of the gauge link (in positive as well as negative direction, i.e., 8 possibilities) and $a,b$ parameterize the row and column of the $\ggrp{SU}{3}$-valued gauge link (row-major).
...
\tldr{clover field indices}
The clover field has indices $(x, \pm, i)$, where $x$ is the lattice index, $\pm \in \{+, -\}$ denotes the chirality (i.e. the upper ($+$) or lower ($-$) $6 \times 6$-block of the $12 \times 12$ clover matrix).
