...
\tldr{mg intro}
In this chapter we will discuss multigrid and its prominent role as a solver preconditioner (\cref{sec:mg:precond}).
After that, we will construct a propagator decomposition in terms of contributions from multigrid levels (\cref{sec:mg:prop}).
...
The setup phase consist of a definition of a bootstrap set $\mathcal{B}$ that consists of $\cNc = 10-30$ approximate low modes, a description of a coarse lattice (\cref{sec:lc:coarse}) and spin degrees of freedom (\cref{sec:lc:spin}).
...
Multigrid is meant to be applied recursively by introducing multiple coarse lattices called \df{levels}.
...
The relaxed formulation allows coarse levels that are not valid coarsenings of each other, for instance \Ln{2} and \Ln{3} in \cref{tab:mg:formulations:example}, but all are valid coarsenings of the fine grid.
...
\tldr{recursive vs loop: different features regarding smoothing and flexibility}
