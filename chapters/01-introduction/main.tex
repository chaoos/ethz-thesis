...
An additional term proportional to $a$ was introduced later~\cite{sw1985} to cancel the $\bigO(a)$ discretization effects introduced by the Wilson-term systematically, while still eliminating the doublers,
...
The local electromagnetic vector current is defined as \worktodo{maybe add flavor index, also later in \cref{eq:a:mu:lohvp} such that if looks like the flavor decomposition?}
\begin{align}
...
In addition to the \ggrp{SU}{3}-valued QCD gauge field $U_\mu(x)$, we have the compact \ggrp{U}{1}-valued QED gauge field $A_\mu(x)$ which when combined, results in a \ggrp{U}{3}-valued field $W_{\mu}(x) = e^{i q A_\mu(x)} U_\mu(x)$ with $q$ the charge of a quark.
...
First results suggest that simulating QCD+QED directly, instead of adding IB effects perturbatively on the top of QCD-only results, can be beneficial for the uncertainty of the IB term~\citeOwn{qcd_qed_paola}.
