...
\tldr{interface intro}
This section documents the code required for interfacing \openqxd with \quda and describes design choices made.
A working interface requires substantial code additions and changes in both applications.
This chapter is divided thus into two major sections: developments made in \quda (\cref{sec:interface:quda}) and developments made in \openqxd (\cref{sec:interface:openqxd}).
...
\begin{figure}
  ...
  \caption{Call graph of the \capcode{openQCD\_qudaSolverGetHandle} function. Solid lines imply the function will always call the function the arrow points to, dashed lines will call only if necessary. The graph divides into namespaces in \openqxd (top) to obtain latest parameters and revision counter, the interface (center) and \quda (bottom).}
...
#define QUDA_L3 L3
\end{minted}
\caption{Excerpt of the file \capcode{include/quda\_global.h} describing the (second) dual process grid. Its default values are equal to the native process grid and local lattice sizes from \capcode{include/global.h}, compare \cref{lst:openqxd:global.h}.}
