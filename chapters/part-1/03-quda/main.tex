...
We go through an example of a simple kernel and how it is being called \quda-internally (\cref{sec:quda:kernel:invocation}), followed by how lattice fields are abstracted (\cref{sec:quda:fields}).
...
We finish with a list of important functions exposed via \qudas header file (\cref{sec:quda:interface}) and a brief summary (\cref{sec:quda:summary}).

\tldr{what is quda}
\Quda \cite{QUDApaper} is a library written in C++ that contains a suite of efficient kernels and solvers to implement lattice QCD simulations. It can be used as a plug and play substitute for certain compute intensive tasks.
...
It thus runs efficiently on a variety of different hardware architectures and vendors.
...
%This makes sense in memory-bound scenarios, because, if the field is represented in a compressed format, memory traffic produced by kernels reading such a field is decreased whereas the arithmetic intensity is increased.
...
In the following, we will list interface functions exposed by \qudas interface via \code{quda.h} that are relevant in the remainder of this document, especially for the discussion of the interface, \cref{ch:p1:interface}.
